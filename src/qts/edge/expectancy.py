...
def compute_expectancy(trades_pnl: list[float], costs_per_trade: float = 0.0) -> ExpectancyReport:
    ...
    pf = float(wins.sum() / -losses.sum()) if len(losses)>0 and losses.sum()!=0 else float('inf') if wins.sum()>0 else 0
    ...
    return ExpectancyReport(expectancy, pf, avg_win, avg_loss, win_rate, loss_rate, dd, es, turnover, cost, expectancy, net_exp, len(arr))
...
def evaluate_minimum_economic_edge(expectancy: float, cost: float, uncertainty: float, penalty: float, threshold: float = 0.0) -> EconomicEdge:
