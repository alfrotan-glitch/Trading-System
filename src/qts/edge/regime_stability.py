...
from qts.regime.detector import VolatilityRegimeDetector, Regime
...
def evaluate_regime_stability(bars: list[Bar], equity_curve: np.ndarray, window: int = 20) -> list[RegimeResult]:
    ...
    rets = np.diff(closes) / closes[:-1] if len(closes)>1 else np.array([0])
    # Vol detector
    vol_det = VolatilityRegimeDetector(window=window)
    vol_regime = vol_det.label(bars)
    # Trend vs range: if |SMA20 - SMA50| / price >0.02 => trend else range
    results = []
    # Split into 3 equal regimes for demo
    n = len(bars)
    for name, sl in [("trend", slice(0, n//3)), ("range", slice(n//3, 2*n//3)), ("high_vol", slice(2*n//3, n))]:
        ...
        max_dd = float((np.maximum.accumulate(eq_slice) - eq_slice).max() / np.maximum.accumulate(eq_slice).max()) if len(eq_slice)>0 else 0.0
