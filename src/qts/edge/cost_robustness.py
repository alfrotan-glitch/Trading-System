...
def evaluate_cost_robustness(equity_gross: np.ndarray, equity_net: np.ndarray, trades: int, avg_spread_bps: float = 3.0) -> CostRobustnessResult:
...
def run_conservative_cost_scenarios(backtest_engine, instrument, timeframe, data_version, strategy_id, params):
    ...
    for s in spreads:
        try:
            res = backtest_engine.run(instrument, timeframe, data_version, strategy_id=strategy_id, strategy_params=params)
