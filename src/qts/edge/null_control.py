...
import random
import numpy as np
from qts.domain.value_objects import Bar, Signal, Side
...
class NullControl:
    ...
    def randomized_timing(self, bars: list[Bar]) -> list[Signal]:
        ...
        signals = []
        for b in random.sample(bars, min(5, len(bars)//10)):
            side = random.choice([Side.BUY, Side.SELL])
            signals.append(Signal(instrument=b.instrument, side=side, strength=0.7, event_time=b.close_time, hypothesis_id="H-NULL-TIMING", strategy_id="null_timing"))
        return signals
    ...
    def shuffled_labels(self, bars: list[Bar], real_signals: list[Signal]) -> list[Signal]:
        ...
        out = []
        for s, new_side in zip(real_signals, sides):
