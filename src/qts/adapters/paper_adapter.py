...
class RealisticPaperBroker(BrokerAdapter):
    ...
    def submit(self, intent: OrderIntent) -> Order:
        ...
        if est_price is None:
            ...
            if tick:
                ...
                est_price = Decimal("2000")  # fallback for backtest where ticks not available
        ...
        if required_margin > account.free_margin + Decimal("0.01"):
            raise ValueError(f"insufficient free margin: need {required_margin:.2f} have {account.free_margin:.2f} for {intent.instrument.symbol} qty {normalized_qty} price {est_price}")
    ...
    def apply_fill(self, fill) -> None:
        ...
        if fill.client_order_id in self._orders:
            ...
            self._orders[fill.client_order_id] = o.model_copy(update={"state": OrderState.FILLED, "filled_quantity": fill.quantity, "avg_fill_price": fill.price})
    ...
    def account(self) -> Account:
        ...
        for pos in self._positions.values():
            ...
            if price:
                ...
                mid = pos.avg_price if pos.avg_price != 0 else Decimal("2000")
