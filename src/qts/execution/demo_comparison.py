...
def compare_paper_shadow_demo(
    ...
    if not paper_fills and Path("data/evidence/paper_trades.json").exists():
        try:
            paper_fills = json.loads(Path("data/evidence/paper_trades.json").read_text()).get("fills", [])
        except Exception:
            paper_fills = []
    if not shadow_intents and Path("data/evidence/shadow_intents.json").exists():
        try:
            shadow_intents = json.loads(Path("data/evidence/shadow_intents.json").read_text()).get("intents_sample", [])
        except Exception:
            shadow_intents = []
    if not demo_observations and Path("data/evidence/demo_forward_observations.json").exists():
        try:
            demo_observations = json.loads(Path("data/evidence/demo_forward_observations.json").read_text()).get("observations", [])
        except Exception:
            demo_observations = []
    ...
    if paper_fills and shadow_intents:
        ...
        signal_agreement = min(len(paper_fills), len(shadow_intents)) / max(len(paper_fills), len(shadow_intents)) if max(len(paper_fills), len(shadow_intents))>0 else 0.0
    ...
    pnl_paper = sum(float(f.get("pnl", 0)) for f in paper_fills) if paper_fills and isinstance(paper_fills[0], dict) else 0.0
