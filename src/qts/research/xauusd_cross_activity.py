"""
H-XF-02: high volatility × activity magnitude (High+Active vs High+Quiet) at 256/1024.

Locked: trail16 high >=0.20, active gap 0<gap<=100, quiet gap>500.
"""
...
SCHEMA = "qts.xauusd_cross_activity.v1"
PREREGISTRATION = "docs/xauusd_cross_next_step_H-XF-02_2026-09-23.md"
HYPOTHESIS = "H-XF-02"
...
class CrossActivityScan:
    ...
    def add_batch(self, bid: np.ndarray, ask: np.ndarray, time_msc: np.ndarray) -> None:
        ...
        for h in HORIZONS:
            ...
            # also need anchor-1 >= base for gap
            mask_history &= anchors - 1 >= base
            ...
            gap_i = all_stamps[local] - all_stamps[local - 1]
            is_high = trail_abs >= HIGH_TRAIL - 1e-9
            is_active = (gap_i > 0) & (gap_i <= ACTIVE_MAX)
            is_quiet = gap_i > QUIET_MIN
            is_A = is_high & is_active
            is_B = is_high & is_quiet
            ...
            # need delay abs for gate
            has_next = exit_ + 1 < len(all_mid)
            abs_delay = np.where(has_next, np.abs(all_mid[exit_ + 1] - all_mid[local + 1]) * 0.005, np.nan)
        ...
        tail = min(max(HORIZONS) + TRAIL + 3, len(all_stamps))
...
def _compute_stats(is_A, is_B, abs_move, exceed):
    ...
    return {"n_A": n_a, "n_B": n_b, "mean_A": mean_a, "mean_B": mean_b, "ratio": ratio, "gap_dollars": gap, "p_A": p_a, "p_B": p_b, "lift": lift}


def evaluate_cross_activity(scan: CrossActivityScan, *, complete_rows: int = DISCOVERY_ROWS) -> dict[str, Any]:
    def aggregate(h: int):
    ...
    for t in range(3):
        ...
        if not np.any(mask):
            tercile_stats.append({"tercile": t, "n_A": 0, "n_B": 0, "mean_A": None, "mean_B": None, "ratio": None, "gap_dollars": None, "p_A": None, "p_B": None, "lift": None, "blocks": 0})
            continue
    ...
    output: dict[str, Any] = {
        ...
        "measured": {"256": primary_stats, "1024": stability_stats, "terciles": tercile_stats, "gap_under_one_hour": gap_stats, "delay": delay_stats, "blocks_per_tercile": blocks_per_tercile, "exclusions": {"note": "non-High or Normal/0 gap ignored"}},
        "hypotheses": {},
    ...
    if underpowered:
        ...
        status = "REJECTED"
        reasons.append(f"floor miss: ratio {primary_stats['ratio']:.3f} < {MIN_RATIO} or gap ${primary_stats['gap_dollars']:.3f} < ${MIN_GAP} or lift {primary_stats['lift']:.3f} < {MIN_LIFT}" if primary_stats and primary_stats["ratio"] is not None else "floor miss")
