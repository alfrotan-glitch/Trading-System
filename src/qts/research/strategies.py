...
from qts.research.strategy import SmaBreakoutStrategy  # re-export
...
class TrendFollowingStrategy:
    ...
    def __init__(self, instrument: Instrument, fast: int = 10, slow: int = 20, ma_type: str = "sma", strategy_id: str = "trend_sma"):
        if fast >= slow:
    ...
    def on_bar(self, bar: Bar) -> list[Signal]:
        ...
        if prev_fast <= prev_slow and fast > slow:
            return [Signal(instrument=bar.instrument, side=Side.BUY, strength=0.7, event_time=bar.close_time, hypothesis_id="H-TREND", strategy_id=self.strategy_id, features={"fast": float(fast), "slow": float(slow)})]
        if prev_fast >= prev_slow and fast < slow:
            return [Signal(instrument=bar.instrument, side=Side.SELL, strength=0.7, event_time=bar.close_time, hypothesis_id="H-TREND", strategy_id=self.strategy_id, features={"fast": float(fast), "slow": float(slow)})]
        return []
...
class BreakoutStrategy:
    ...
    def on_bar(self, bar: Bar) -> list[Signal]:
        ...
        if bar.close > highest:
            return [Signal(instrument=bar.instrument, side=Side.BUY, strength=0.8, event_time=bar.close_time, hypothesis_id="H-BREAKOUT", strategy_id=self.strategy_id, features={"highest": float(highest), "close": float(bar.close)})]
        if bar.close < lowest:
            return [Signal(instrument=bar.instrument, side=Side.SELL, strength=0.8, event_time=bar.close_time, hypothesis_id="H-BREAKOUT", strategy_id=self.strategy_id, features={"lowest": float(lowest), "close": float(bar.close)})]
        return []
...
class MeanReversionStrategy:
    ...
    def on_bar(self, bar: Bar) -> list[Signal]:
        ...
        if close_f > upper:
            return [Signal(instrument=bar.instrument, side=Side.SELL, strength=0.6, event_time=bar.close_time, hypothesis_id="H-MEANREV", strategy_id=self.strategy_id, features={"sma": float(sma), "upper": upper, "lower": lower})]
        if close_f < lower:
            return [Signal(instrument=bar.instrument, side=Side.BUY, strength=0.6, event_time=bar.close_time, hypothesis_id="H-MEANREV", strategy_id=self.strategy_id, features={"sma": float(sma), "upper": upper, "lower": lower})]
        return []
...
class MomentumStrategy:
    ...
    def __init__(self, instrument: Instrument, lookback: int = 10, threshold: float = 0.0, strategy_id: str = "momentum"):
    ...
    def on_bar(self, bar: Bar) -> list[Signal]:
        ...
        if mom > self.threshold:
            return [Signal(instrument=bar.instrument, side=Side.BUY, strength=min(1.0, abs(mom) / 10), event_time=bar.close_time, hypothesis_id="H-MOM", strategy_id=self.strategy_id, features={"mom": mom})]
        if mom < -self.threshold:
            return [Signal(instrument=bar.instrument, side=Side.SELL, strength=min(1.0, abs(mom) / 10), event_time=bar.close_time, hypothesis_id="H-MOM", strategy_id=self.strategy_id, features={"mom": mom})]
        return []
...
class VolatilityStrategy:
    ...
    def __init__(self, instrument: Instrument, atr_period: int = 14, multiplier: float = 1.5, strategy_id: str = "vol_atr"):
    ...
    def on_bar(self, bar: Bar) -> list[Signal]:
        ...
        if bar_range > self.multiplier * atr:
            ...
            return [Signal(instrument=bar.instrument, side=side, strength=0.65, event_time=bar.close_time, hypothesis_id="H-VOL", strategy_id=self.strategy_id, features={"atr": atr, "range": bar_range})]
        return []
...
def create_strategy(family: StrategyFamily, instrument: Instrument, params: dict[str, Any], strategy_id: str | None = None) -> Any:
...
def describe_features(family: StrategyFamily, params: dict[str, Any]) -> dict[str, Any]:
    ...
    if family == StrategyFamily.TREND:
        base["features"] = {"sma_fast": params.get("fast"), "sma_slow": params.get("slow"), "crossover": "fast crosses slow"}
        ...
        base["features"] = {"bb_period": params.get("period"), "k": params.get("k"), "reversion": "close outside Bollinger"}
        ...
        base["features"] = {"atr_period": params.get("atr_period"), "multiplier": params.get("multiplier"), "volatility_breakout": "range > mult*ATR"}
    return base
