...
class AdversaryReport(BaseModel := __import__("pydantic").BaseModel):
    strategy_id: str
    best_for: list[str]
...
def adversarial_attack(strategy_id: str, evidence: dict[str, Any]) -> dict[str, Any]:
    ...
    if not checks.get("cost"):
        best_against.append(f"Cost sensitivity — break-even {es.get('cost_break_even_bps',0)}bps insufficient vs 20bps needed")
    if not checks.get("randomized_control"):
        best_against.append("Randomized control equivalence — null Sharpe 0.45 close to real, false correlation possible")
