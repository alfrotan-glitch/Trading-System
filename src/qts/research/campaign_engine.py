...
from qts.research.statistical import white_reality_check, hansen_spa
...
def run_autonomous_campaign(
    ...
    prior_failures = mem.list_failures(limit=10)
    ...
    plan = {
        ...
        "budget": {"max_trials": max_trials, "max_runtime_s": max_runtime_s, "max_feature_count": 6, "max_param_combinations": max_trials, "max_mutation_depth": 2, "max_retries": 1, "max_data_scope": data_version, "seed": seed},
        "mechanisms": ["trend persistence", "breakout failure", "volatility expansion", "range compression", "time-of-day", "event shock"],
    }
    ...
    cfg = CampaignConfig(name=name, symbol=symbol, timeframe=timeframe, data_version=data_version, family="trend", max_trials=min(6, max_trials), max_runtime_s=max_runtime_s/2, max_param_combinations=min(6, max_trials), seed=seed)
    ...
    if max_trials > 6:
        cfg2 = CampaignConfig(name=name+"-invented", symbol=symbol, timeframe=timeframe, data_version=data_version, family="volatility", max_trials=min(6, max_trials-6), max_runtime_s=max_runtime_s/2, max_param_combinations=min(6, max_trials-6), seed=seed+1)
    ...
    for s in surviving[:2]:
        ...
        params = s.get("params", {})
        mutated = {k: (v+1 if isinstance(v, int) else v*1.1 if isinstance(v, float) else v) for k,v in params.items()}
        refinements.append({"original": s["strategy_id"], "mutated_params": mutated, "reason": "refine surviving via small param mutation"})
    ...
    if refinements and max_trials > len(summary.get("trials", [])) + len(refinements):
        ...
        retest_summary = {"retested": len(refinements), "note": "re-test would run bounded experiments with mutated params, DSR penalty increased"}

    # 11 produce evidence portfolio
    # Novelty
    trials_for_novelty = [{"family": t.get("strategy_id","").split("_")[0], "mechanism": t.get("strategy_id",""), "params": t.get("params",{}), "feature_lineage": []} for t in summary.get("trials", [])]
    ...
    block = any(v.startswith("YES") for v in self_audit.values()) or "UNKNOWN" in str(self_audit.values())
    ...
    for t in eliminated[:5]:
        ...
        entry = MemoryEntry(hypothesis_id=t.get("trial_id",""), strategy_id=t.get("strategy_id",""), family=t.get("strategy_id","").split("_")[0], mechanism="autonomous", params=t.get("params",{}), failed_stage="DSR/PBO", reason=";".join(attacks.get(t.get("strategy_id",""), {}).get("best_evidence_against", [])[:2])[:200] if t.get("strategy_id") in attacks else "scientific gate failure")
        mem.remember(entry)

    # Write machine-readable
    Path("data/evidence/autonomous_campaign.json").write_text(json.dumps({"summary": summary, "plan": plan, "data_review": data_review, "evidence_portfolio": evidence_portfolio, "novelty": novelty, "attacks": attacks, "self_audit": self_audit}, indent=2, default=str))
    return {"plan": plan, "summary": summary, "evidence_portfolio": evidence_portfolio, "novelty": novelty, "self_audit": self_audit}
