...
def white_reality_check(returns: np.ndarray, benchmark: float = 0.0, n_bootstrap: int = 1000, seed: int = 42) -> dict:
    ...
    return {"observed_max": obs_max, "p_value": p_value, "n_bootstrap": n_bootstrap, "purpose": "White Reality Check — is best strategy just luck?", "limitations": "needs large T, assumes stationary"}
...
def hansen_spa(returns_list: list[np.ndarray], benchmark: float = 0.0, n_bootstrap: int = 1000, seed: int = 42) -> dict:
    ...
    return {"observed_spa": obs_spa, "p_value": p_value, "purpose": "Hansen SPA — superior predictive ability", "limitations": "bootstrap assumes no temporal dependence"}
...
def permutation_test(returns: np.ndarray, n_perm: int = 1000, seed: int = 42) -> dict:
    ...
    return {"observed_sharpe": obs_sr, "p_value": p_value, "purpose": "Permutation — is Sharpe just random ordering?", "n_perm": n_perm}
...
def drawdown_distribution(equity: np.ndarray, n_bootstrap: int = 500, seed: int = 42) -> dict:
    ...
    return {"observed_max_dd": float(obs_dd), "bootstrap_mean_dd": float(np.mean(boot_dds)), "bootstrap_95_dd": float(np.percentile(boot_dds, 95)), "expected_shortfall": float(np.mean([d for d in boot_dds if d >= np.percentile(boot_dds, 95)])) if boot_dds else 0.0}
...
def parameter_surface_analysis(param_sharpes: dict[str, float]) -> dict:
    ...
    return {"mean": mean, "std": std, "peak": peak, "stability": stability, "purpose": "Detect overfit to specific params"}
