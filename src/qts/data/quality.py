...
def validate_bars(bars: list[Bar]) -> DataQualityReport:
    ...
    # 4. OHLC invariants
    ohlc_ok = all(b.high >= b.low and b.high >= b.open and b.high >= b.close and b.low <= b.open and b.low <= b.close for b in bars)
    checks.append(QualityCheck("ohlc_invariants", ohlc_ok, "" if ohlc_ok else "high < low or open/close outside high-low"))
    # 5. impossible prices (price <=0, high <0)
    price_ok = all(b.open > Decimal("0") and b.high > Decimal("0") and b.low > Decimal("0") and b.close > Decimal("0") for b in bars)
    ...
    sym_ok = len(set((b.instrument.symbol, b.instrument.venue) for b in bars)) == 1
    ...
    if len(bars) >= 2:
        ...
        gaps = sum(1 for d in deltas if d > common * 1.5 and d < 86400*4)  # ignore >4 days (long closure)
    ...
    if stale_count > len(bars) * 0.1:
        stale_ok = False
    checks.append(QualityCheck("no_broker_artifacts", stale_ok, f"{stale_count} stale zero-volume repeats" if not stale_ok else ""))
...
def dataset_missing_stats(bars: list[Bar], timeframe: str = "1H") -> dict:
    ...
    return {"expected": expected, "actual": len(bars), "missing": missing, "gap_count": gap_count, "max_gap_s": max_gap, "missing_pct": round(missing/expected*100,2) if expected else 0, "timezone": "UTC"}
