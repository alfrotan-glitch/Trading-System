...
| Data provenance and quality | **Strong for the registered bar datasets.** The REAL 15m dataset passed its 12/12 quality checks; provenance, gaps, licensing limits and field semantics are documented. | `docs/data_provenance_xauusd_dukascopy.md`, `docs/market_data_observatory.md` |
...
| REAL impulse research | **Executed under the existing preregistered design.** The locked partition was untouched; no candidate was promoted. | `data/evidence/impulse_research_xauusd_dukascopy_15m.json` |
...
**State:** completed by this documentation/state-convergence change. It
establishes one status authority and labels older snapshots rather than
rewriting historical evidence.

### B. Next engineering/operator milestone — real MT5 observation

Run the existing FO-R1 order-free protocol on a real Windows/MT5 demo terminal,
after the documented readiness checks pass. Preserve the canonical SQLite store,
acquisition ledger, snapshot/export lineage and safety assertions. This produces
observation evidence only; it does not produce execution evidence, a profitable
edge, or permission to submit orders.

No real MT5 session is currently present, so this is an operator/environment
milestone rather than a claim that the repository has already met it.
