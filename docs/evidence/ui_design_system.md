...
**Safety understandable:** DEMO readiness permits observation only. DEMO_EXECUTION is `DISABLED BY POLICY` by default and `ENABLED_AUTHORIZED` only with a recorded owner authorization — the UI reports which one applies (`DISABLED BY POLICY` / `AUTHORIZED · NOT PERMITTED` / `PERMITTED · DEMO ONLY`) and never offers a control that creates order permission. LIVE shows `LOCKED` vs `ELIGIBLE · STILL GATED` vs `UNAVAILABLE`, never enabled. DEMO and LIVE remain unmistakable.
...
3. `modeInfo` — DEVELOPMENT/PAPER/SHADOW cannot submit; DEMO_FORWARD is observation-only; DEMO_EXECUTION is disabled by policy unless a recorded owner authorization resolves it to ENABLED_AUTHORIZED, and orders remain individually gated; LIVE is gated and locked.
