...
| DEMO execution | **Authorized but not trading.** An owner authorization artifact (DEMO only, `LIVE` locked, zero real capital) resolves `DEMO_EXECUTION = ENABLED_AUTHORIZED`; the shipped default of a checkout without that artifact remains `DISABLED BY POLICY`. Orders additionally require the staged progression, a pinned+confirmed broker identity, an eligible registered strategy and a passing `run_pretrade_gate`. Current trading state: `NO_TRADE`, `orders_submitted=0`. | `qts.lifecycle.demo_authorization`, `qts.lifecycle.demo_authority`, `qts.execution.demo_pretrade`, `docs/demo_execution_authorization_and_safety_2026-09-23.md` |
