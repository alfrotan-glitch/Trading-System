...
| Market-making | Does low-vol tight spread capture spread via limit fills with smaller adverse excursion than high-vol wide? | **H-MM-01 REJECTED by absolute-net gate but shows risk edge.** [Preregistration](xauusd_marketmaking_next_step_H-MM-01_2026-09-23.md); same verifiable 70.78M view; Actions `35864265388` — `MarketMakingScan` `F=Low(≤0.10)+Tight(≤0.20)` vs `U=High(≥0.20)+Wide(≥0.27)`, front-of-queue SYNTHETIC. 16q both-filled: F `332,077` **78.2%** vs U `634,897` **72.8%**, **lift 0.054** (pass ≥0.05); 256q excursion mean F **$0.498** vs U **$1.308**, **ratio 2.62** (pass), **gap $0.809** (pass), max F **$0.758** vs U **$1.969** (F smaller); terciles lifts 0.054/0.029/0.029 (2 <0.05 but >0) ratios 1.52/2.30/1.25 all >1; gap/delay sign pass. **Net per both-filled F $0.132 vs U $0.239** — F < U because tight spread (20c vs 27c) gives smaller absolute capture, so `net_F > net_U` floor fails → REJECTED. Risk-adjusted (net/max) F **0.174** vs U **0.121** and lift+ratio pass. `reports/xauusd_marketmaking_state.json`. |
