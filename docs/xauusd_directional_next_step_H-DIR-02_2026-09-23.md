...
| View to be built | None exists | `data/raw/mt5_ticks_discovery_verifiable/` — `manifest.json` (`qts.xauusd_discovery_view.v1`, `COMPLETE_DISCOVERY_ONLY`) + `parts/part-*.parquet` (only whole, non-straddling parts), plus independently pinned `docs/evidence/xauusd_directional_view_authority_H-DIR-02.json` (`qts.xauusd_discovery_view_authority.v1`) |
...
1. The view manifest `view_manifest_sha256` is pinned in `docs/evidence/xauusd_directional_view_authority_H-DIR-02.json` **before** any H-DIR-02 run. No view is trusted on its self-label.
...
Every REJECTED/INCONCLUSIVE outcome on H-DIR-02 will be preserved in `docs/evidence/xauusd_edge_discovery_map.md` and the cycle closure, and will not be rerun with loosened floors, altered horizons, or favorable-subset selection. The 15-minute Dukascopy completeness blocker and the original H-DIR-01 BLOCK remain independent, preserved failures.
