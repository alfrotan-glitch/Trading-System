...
def test_inventory_26_fields():
    ...
    assert len(inv) >= 2
    required_fields = ["source","provider","instrument","timeframe","date_range","row_count","tick_count","timezone","timestamp_resolution","ohlc_availability","bid_availability","ask_availability","spread_availability","volume_availability","tick_volume_vs_real_volume","missingness","duplicates","gaps","session_coverage","market_closure_handling","broker_artifacts","checksum","ingestion_method","preprocessing_version","provenance","schema_version","quality_report","curated_path","raw_preserved","research_eligibility"]
    for entry in inv:
...
def test_data_source_catalog_5_providers():
    ...
    assert len(cat) == 5
    for prov in cat:
        for k in ["provider_id","description","historical_depth","granularity","bid_ask","tick","timezone","licensing","api","reliability","quality","symbol_mapping","timestamp_behavior","limitations","cost","suitability_research","suitability_execution"]:
...
def test_tick_canonical_model_distinguishes_real_synthetic():
    ...
    # REAL tick
    t_real = Tick(instrument=instr, bid=Decimal("2000"), ask=Decimal("2000.5"), last=Decimal("2000.25"), bid_size=Decimal("1"), ask_size=Decimal("1"), event_time=now, tick_type="quote", session="London", source="REAL")
    assert t_real.source == "REAL"
    assert t_real.tick_type == "quote"
    # SYNTHETIC tick must be labeled
    t_synth = Tick(instrument=instr, bid=Decimal("2000"), ask=Decimal("2000.5"), event_time=now, source="SYNTHETIC", tick_type="quote", session="NY")
...
def test_execution_reality_records_signal_expected_actual_and_labels_source():
    ...
    now = datetime.now(UTC)
    obs = ExecutionObservation(symbol="XAUUSD", signal_price=Decimal("2000"), expected_price=Decimal("2000.1"), requested_price=Decimal("2000.1"), submission_timestamp=now, broker_ack_timestamp=now+timedelta(milliseconds=100), fill_timestamp=now+timedelta(milliseconds=150), requested_volume=Decimal("0.1"), filled_volume=Decimal("0.1"), realized_price=Decimal("2000.2"), bid_at_decision=Decimal("2000"), ask_at_decision=Decimal("2000.2"), spread_at_decision=Decimal("0.2"), market_state="open", source="REAL")
    ...
    synth = ExecutionObservation(symbol="XAUUSD", signal_price=Decimal("2000"), expected_price=Decimal("2000"), requested_price=Decimal("2000"), submission_timestamp=now, requested_volume=Decimal("0.1"), source="SYNTHETIC")
    store.record(synth)
...
def test_data_quality_adversarial_fail_closed():
    ...
    bars = [Bar(instrument=instr, open=Decimal("2000"), high=Decimal("2001"), low=Decimal("1999"), close=Decimal("2000.5"), volume=Decimal("1000"), open_time=now+i*timedelta(hours=1), close_time=now+(i+1)*timedelta(hours=1), data_version="v1", source="test") for i in range(5)]
    # Valid passes
    ...
    # Zero price fails
    bad = Bar(instrument=instr, open=Decimal("0"), high=Decimal("1"), low=Decimal("0"), close=Decimal("0"), volume=Decimal("1000"), open_time=now+timedelta(hours=10), close_time=now+timedelta(hours=11), data_version="v1", source="test")
...
def test_bid_ask_inversion_stale_outoforder_future_fail_closed():
    from qts.data.quality import validate_bars
    from qts.domain.value_objects import Instrument, Bar, Tick
    from datetime import UTC, datetime, timedelta
    from decimal import Decimal
    instr = Instrument(symbol="XAUUSD")
    now = datetime.now(UTC)
    # Timestamp shift (future leakage)
    future = Bar(instrument=instr, open=Decimal("2000"), high=Decimal("2001"), low=Decimal("1999"), close=Decimal("2000.5"), volume=Decimal("1000"), open_time=datetime.now(UTC)+timedelta(days=1), close_time=datetime.now(UTC)+timedelta(days=1, hours=1), data_version="v1", source="test")
...
def test_versioning_immutable_new_version_on_preprocessing_change():
    ...
    bars2 = [Bar(instrument=instr, open=b.open+Decimal("0.01"), high=b.high+Decimal("0.01"), low=b.low+Decimal("0.01"), close=b.close+Decimal("0.01"), volume=b.volume, open_time=b.open_time, close_time=b.close_time, data_version=b.data_version, source=b.source) for b in bars]
...
def test_no_fake_data_rule_distinguish():
    ...
    for entry in inv:
        ...
        assert "SYNTHETIC" in entry["tick_volume_vs_real_volume"] or "tick" in entry["tick_volume_vs_real_volume"].lower()
    # Execution reality must have source field
    from qts.execution.reality import ExecutionObservation
    from decimal import Decimal
    from datetime import UTC, datetime
    now = datetime.now(UTC)
    real = ExecutionObservation(symbol="XAUUSD", signal_price=Decimal("2000"), expected_price=Decimal("2000"), requested_price=Decimal("2000"), submission_timestamp=now, requested_volume=Decimal("0.1"), source="REAL")
    synth = ExecutionObservation(symbol="XAUUSD", signal_price=Decimal("2000"), expected_price=Decimal("2000"), requested_price=Decimal("2000"), submission_timestamp=now, requested_volume=Decimal("0.1"), source="SYNTHETIC")
...
def test_research_quality_gate_blocks_if_insufficient():
    ...
    inv = json.loads(p_inv.read_text())
    ...
    symbols = set(e["instrument"] for e in inv)
    assert len(symbols) == 1
    # Execution realism 0 real obs
    p_exec = Path("data/evidence/execution_reality.json")
    exec_sum = json.loads(p_exec.read_text()) if p_exec.exists() else {"count":0}
...
def test_desktop_5_views_exist():
    ...
    html = Path("src/qts/desktop/ui/index.html").read_text()
...
def test_clean_room_reproducibility():
    ...
    cfg = CampaignConfig(name="repro-test", symbol="XAUUSD", timeframe="1H", data_version="20260916-010-572728d9", family="trend", max_trials=4, max_runtime_s=20, seed=42)
    r1 = run_campaign(cfg)
    r2 = run_campaign(cfg)
...
def test_provenance_never_overwritten():
    ...
    inv = json.loads(p.read_text())
    for entry in inv:
        assert "never" not in entry["ingestion_method"].lower() or "raw storage" in entry["ingestion_method"].lower() or "Provider" in entry["ingestion_method"]
