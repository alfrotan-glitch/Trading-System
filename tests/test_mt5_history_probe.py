...
def test_probe_reports_bid_ask_fields_without_orders() -> None:
    ...
    assert all(window["analysis_status"] == "DEFERRED_TO_LOCAL_DATASET" for window in report["windows"])
    assert all("raw_rows_sha256" not in window for window in report["windows"])
