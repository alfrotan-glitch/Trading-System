...
def cli_workspace(tmp_path, monkeypatch):
    """Isolated workspace for CLI-invoking tests.

    Root causes addressed:
    * CLI tests used to run against the repository's real ./data directory,
      mutating TRACKED files (data/evidence/*.json) and depending on a
      hardcoded, date-derived version id ("2026...-010-...") that only exists
      if a previous test happened to ingest the fixture the same day.
    * Here the CLI runs in a temp CWD with its own data root, bootstrapped
      deterministically from the committed SYNTHETIC fixture. The returned
      BootstrapResult carries the resolved version id — never hardcode one.
    """
    import shutil
    ...
    shutil.copy(
        REPO_ROOT / "data" / "fixtures" / "XAUUSD_1H_500.csv",
        tmp_path / "data" / "fixtures" / "XAUUSD_1H_500.csv",
    )
    if (REPO_ROOT / "configs").exists():
        shutil.copytree(REPO_ROOT / "configs", tmp_path / "configs")
    res = bootstrap_data(root=tmp_path / "data", fixture=tmp_path / "data" / "fixtures" / "XAUUSD_1H_500.csv")
    assert res.ok, f"bootstrap failed in cli_workspace: {res.messages}"
    return res
...
def test_market_data_spread_limit():
    ...
    mock.symbol_info_tick.return_value = MagicMock(
        bid=1980, ask=2020, time=datetime.now(UTC).timestamp()
    )  # 40/2000=2% 200bps
    adapter = MT5Adapter(mt5_module=mock)
...
def test_market_data_symbol_identity():
    ...
    mock.symbol_info_tick.return_value = MagicMock(bid=1999.5, ask=2000.5, time=datetime.now(UTC).timestamp())
    adapter = MT5Adapter(mt5_module=mock)
    _md = MarketDataProvider(adapter)
    _instr = Instrument(symbol="XAUUSD")
...
def test_market_data_correct_side_and_persist():
    ...
    intent = OrderIntent(
        instrument=instr, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="side_test", strategy_id="s"
    )
    ...
    _risk_veto = [e for e in audit2.events if e.event_type.value == "RiskVeto"]
...
def test_order_submission_canonical_id_normalized_quantity_price():
    ...
    intent = OrderIntent(
        instrument=instr, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="a" * 40, strategy_id="s"
    )
...
def test_order_submission_broker_response_mapping():
    ...
    bar = _bar()
    intent = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="resp1", strategy_id="s"
    )
    ...
    intent2 = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="resp2", strategy_id="s"
    )
    ...
    intent3 = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="resp3", strategy_id="s"
    )
...
def test_broker_timeout_after_acceptance_ambiguous():
    ...
    bar = _bar()
    intent = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="amb_after", strategy_id="s"
    )
    ...
    intent2 = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="next", strategy_id="s"
    )
...
def test_duplicate_after_restart():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        eng2 = ExecutionEngine(
            om2, risk2, broker, MatchingEngine(), Portfolio(initial_balance=Decimal("10000")), audit=audit2, db_path=db
        )
        order2, fills2 = eng2.submit_intent(intent, bar=bar)
        assert fills2 == []
...
def test_reconciliation_after_ambiguous():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        intent = OrderIntent(
            ...
            side=Side.BUY,
            quantity=Decimal("0.01"),
            client_order_id="recon_amb1",
            strategy_id="s",
        )
        eng.submit_intent(intent, bar=bar)
        assert eng.is_suspended
        # Reconcile should keep suspended until healed
        _report = eng.reconcile()
    ...
    class StaleAccountBroker(BrokerAdapter):
        ...
        def account(self):
            ...
            return Account(
                ...
                currency="USD",
                updated_at=datetime.now(UTC) - timedelta(seconds=400),
            )
...
def test_real_account_contradictory_blocks():
    class ContradictBroker(BrokerAdapter):
        ...
        def account(self):
            ...
            return Account(
                balance=Decimal("10000"),
                equity=Decimal("10000"),
                margin=Decimal("1000000"),
                free_margin=Decimal("-900000"),
    ...
    bar = _bar()
    intent = OrderIntent(
        instrument=bar.instrument, side=Side.BUY, quantity=Decimal("0.01"), client_order_id="contrad", strategy_id="s"
    )
    order, _ = eng.submit_intent(intent, bar=bar)
    assert order is None
    assert eng.is_suspended
...
def test_no_mocks_in_live_path():
    ...
    mock.account_info.return_value = MagicMock(
        balance=5000, equity=5000, margin=0, margin_free=5000, leverage=100, currency="USD", login=123
    )
    ctx = eng._risk_ctx()
...
def test_reconciliation_status_mismatch():
    ...
    broker._orders["stat_mismatch"] = Order(
        order_id=uuid7(),
        client_order_id="stat_mismatch",
        instrument=instr,
        side=Side.BUY,
...
def test_reconciliation_suspended_persists_across_restart():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        eng3 = ExecutionEngine(
            OrderManager(audit=InMemoryAuditLog(), idempotency=IdempotencyStore(db_path=db)),
...
def test_live_cli_gate_all_prerequisites(cli_workspace):
    ...
    result = runner.invoke(
        main, ["run", "--mode", "live", "--data-version", cli_workspace.version, "--confirm", "live"]
    )
    assert result.exit_code == 2
    assert "live blocked" in result.output.lower()
    # With dry_run, should not block on live gate but succeed
    result2 = runner.invoke(main, ["run", "--mode", "dry_run", "--data-version", cli_workspace.version])
...
def test_live_cli_gate_emits_audit(cli_workspace):
    # Dry-run emits audit
    from click.testing import CliRunner
    ...
    runner = CliRunner()
    runner.invoke(main, ["run", "--mode", "dry_run", "--data-version", cli_workspace.version])
    log = SqliteAuditLog()
...
def test_dry_run_no_submission(cli_workspace):
    from click.testing import CliRunner
...
def test_restart_normal_state():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        intent = OrderIntent(
            ...
            side=Side.BUY,
            quantity=Decimal("0.01"),
            client_order_id="normal_restart",
            strategy_id="s",
        )
        order, _ = eng.submit_intent(intent, bar=bar)
        assert order.state == OrderState.FILLED
...
def test_restart_partially_filled():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        eng = ExecutionEngine(
            ...
            risk,
            broker,
            MatchingEngine(MatchingConfig(partial_fill_model="volume_based")),
            portfolio,
            audit=audit,
            db_path=db,
        )
        ...
        )  # after second fill, it becomes FILLED; but intermediate was PARTIALLY_FILLED
...
def test_restart_filled():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        intent = OrderIntent(
            ...
            side=Side.BUY,
            quantity=Decimal("0.01"),
            client_order_id="filled_restart",
            strategy_id="s",
        )
        order, _ = eng.submit_intent(intent, bar=bar)
        assert order.state == OrderState.FILLED
...
def test_paper_shadow_unchanged(cli_workspace):
    ...
    runner = CliRunner()
    result = runner.invoke(main, ["run", "--mode", "paper", "--data-version", cli_workspace.version])
    assert result.exit_code == 0, result.output
    import json
