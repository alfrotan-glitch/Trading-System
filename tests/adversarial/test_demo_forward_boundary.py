...
def test_live_account_to_demo_blocked():
    class MockAI:
        login=123; server="ICMarkets-Live"; company="ICMarkets"; balance=10000; margin=0; trade_mode=2
    class MockMT5:
        def terminal_info(self): return object()
        def account_info(self): return MockAI()
        def symbol_info(self, sym):
            class SI: contract_size=100; volume_min=0.01; volume_max=100; volume_step=0.01; digits=2; point=0.01; trade_allowed=True; trade_mode=4
            return SI()
        def symbol_info_tick(self, sym):
            class T: bid=2000; ask=2000.5; time=__import__("time").time()
            return T()
...
def test_demo_account_passes_is_demo_check():
    class MockAIDemo:
        login=123; server="ICMarkets-Demo"; company="ICMarkets"; balance=10000; margin=0; trade_mode=0
    class MockMT5:
        def terminal_info(self): return object()
        def account_info(self): return MockAIDemo()
        def symbol_info(self, sym):
            class SI: contract_size=100; volume_min=0.01; volume_max=100; volume_step=0.01; digits=2; point=0.01; trade_allowed=True; trade_mode=4
            return SI()
        def symbol_info_tick(self, sym):
...
def test_stale_tick_blocks():
    import time
    class MockAI:
        login=123; server="Demo"; company="B"; balance=10000; margin=0; trade_mode=0
    class MockMT5:
        def terminal_info(self): return object()
        def account_info(self): return MockAI()
        ...
        def symbol_info_tick(self, s):
            class T: bid=2000; ask=2000.5; time=time.time()-120
            return T()
    rpt = demo_forward_readiness_report(mt5_module=MockMT5())
...
def test_invalid_bid_ask_blocks():
    ...
    class MockMT5:
        ...
        def symbol_info(self, s):
            ...
            return SI()
        def symbol_info_tick(self, s):
            class T: bid=2000.5; ask=2000; time=__import__("time").time()
            return T()
    rpt = demo_forward_readiness_report(mt5_module=MockMT5())
...
def test_excessive_spread_blocks():
    ...
    class MockMT5:
        ...
        def symbol_info(self, s):
            ...
            return SI()
        def symbol_info_tick(self, s):
            class T: bid=2000; ask=2010; time=__import__("time").time()
            return T()
    rpt = demo_forward_readiness_report(mt5_module=MockMT5())
...
def test_insufficient_margin_blocks_account_state():
    class MockAI:
        login=123; server="Demo"; company="B"; balance=0; margin=0; trade_mode=0
    class MockMT5:
        def terminal_info(self): return object()
        def account_info(self): return MockAI()
        def symbol_info(self, s):
...
def test_wrong_lot_size_rejected_by_symbol_spec():
    ...
    class MockSI:
        contract_size=100; volume_min=0.01; volume_max=1.0; volume_step=0.01; digits=2; point=0.01; trade_allowed=True; stops_level=10; freeze_level=0; trade_mode=4
        execution_mode=0; filling_mode=2; trade_tick_size=0.01
    class MockMT5:
        def symbol_info(self, s): return MockSI()
        ...
        def account_info(self):
            class AI: balance=10000; equity=10000; margin=100; free_margin=9900
            return AI()
        def terminal_info(self): return object()
        def order_send(self, req): return type("R", (), {"retcode": 10013, "comment": "Invalid volume"})()
...
def test_partial_fill():
    from qts.domain.value_objects import Order, Instrument, Side, OrderType, OrderState
    instr = Instrument(symbol="XAUUSD")
    order = Order(order_id="O1", client_order_id="C1", instrument=instr, side=Side.BUY, quantity=Decimal("0.2"), order_type=OrderType.MARKET, strategy_id="s1")
