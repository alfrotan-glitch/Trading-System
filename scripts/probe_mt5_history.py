...
def _window_report(mt5: Any, symbol: str, days: int, end_utc: datetime, flags: Any) -> dict[str, Any]:
    ...
    # Capability probing is intentionally bounded.  It records only the
    # response shape and count; row hashing, duplicate analysis, spread
    # statistics, and quality checks belong to the local deferred analyzer.
    first = _row_projection(rows[0], fields) if row_count else None
    last = _row_projection(rows[-1], fields) if row_count else None
    return {
        **request,
        ...
        "first_row_timestamps": {field: first.get(field) for field in ("time", "time_msc") if first and field in first},
        "last_row_timestamps": {field: last.get(field) for field in ("time", "time_msc") if last and field in last},
        "timestamp_basis": "RAW_MT5_FIELDS_ONLY; no UTC normalization or broker-offset inference performed",
        "analysis_status": "DEFERRED_TO_LOCAL_DATASET",
    }
