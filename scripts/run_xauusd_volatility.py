"""Run the preregistered volatility-state scan on the canonical zip.
...
from qts.research.xauusd_volatility import render_volatility_report, scan_dataset, write_report
...
def _fail(output: Path, markdown: Path, status: str, detail: str) -> int:
    ...
    markdown.write_text(render_volatility_report(report) + "\n", encoding="utf-8")
    return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    output.with_name("xauusd_volatility_summary.json").write_text(json.dumps(summary, indent=2) + "\n", encoding="utf-8")
